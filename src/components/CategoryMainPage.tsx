...
export default function CategoryMainPage({ categoryId, title, basePath }: CategoryMainPageProps) {
  ...
  const fetchPosts = useCallback(async () => {
    try {
      ...
      const finalPosts = categoryId === 'ban-doc' ? allPosts : allPosts.slice(0, 3)
      setPosts(finalPosts)
    } catch (error) {
                    ...
                    <div key={post.id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow border border-gray-200 overflow-hidden h-full flex flex-col">
                      <a href={`/ban-doc/${post.slug}`} className="h-full flex">
                        {post.images?.[0]?.image?.path ? (
                          <div className="flex-shrink-0 w-48 h-32 overflow-hidden group">
                            <img
                          ...
                          <div className="flex-shrink-0 w-48 h-32 bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
                        ...
                        <div className="flex-1 p-4 flex flex-col justify-between">
                          <div>
                            ...
                            <h3 
                              className="text-base font-bold text-gray-900 mb-2 leading-tight hover:text-red-600 transition-colors line-clamp-2"
                              title={post.title}
                            >
                              {post.title}
                            </h3>
